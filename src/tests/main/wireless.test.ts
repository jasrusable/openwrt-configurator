...
test("wireless", async (t) => {
  ...
  t.is(apOpenWrtConfig.wireless?.["wifi-device"]?.[0].properties.band, "2g");
  t.is(
    apOpenWrtConfig.wireless?.["wifi-device"]?.[0].properties.path,
    "platform/10300000.wmac"
  );
  t.is(
    apOpenWrtConfig.wireless?.["wifi-device"]?.[0].properties.type,
    "mac80211"
  );
  t.is(apOpenWrtConfig.wireless?.["wifi-device"]?.[1].name, "radio1");
  t.is(apOpenWrtConfig.wireless?.["wifi-device"]?.[1].properties.band, "5g");
  t.is(
    apOpenWrtConfig.wireless?.["wifi-device"]?.[1].properties.path,
  ...
  t.is(
    apOpenWrtConfig.wireless?.["wifi-iface"]?.[0].properties.device,
    "radio0"
  );
  t.is(apOpenWrtConfig.wireless?.["wifi-iface"]?.[0].properties.network, "lan");
});
